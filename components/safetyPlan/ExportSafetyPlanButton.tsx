...
interface ExportSafetyPlanButtonProps {
  medications: Medication[];
  stats: SafetyPlanStats;
}

export default function ExportSafetyPlanButton({
  medications,
  stats,
}: ExportSafetyPlanButtonProps) {
  ...
  const generatePDFForWeb = async (): Promise<void> => {
    ...
    const currentDate = DateUtils.formatDate(new Date(), "short").replace(
      /\//g,
      "-"
    );
  ...
  const generatePDFForMobile = async (): Promise<string | null> => {
    try {
      const htmlContent = SafetyPlanPDFTemplate.generateHTML(
        medications,
        stats
      );
  ...
  const sendEmail = async (pdfUri: string) => {
    try {
      ...
      const patientNames = [
        ...new Set(medications.map((med) => med.patientName)),
      ];
      ...
      const patientNameList =
        sortedNames.length > 1
          ? `${sortedNames.slice(0, -1).join(", ")} and ${
    ...
    >
      ...
      >
        <ThemedText
          ...
          style={[styles.buttonText, isProcessing && styles.buttonTextDisabled]}
          lightColor="#f78b33"
          darkColor="#f78b33"
        >
          {isProcessing
      ...
      {medications.length > 0 && (
        ...
        >
          ...
          >
            {Platform.OS === "web"
              ? `PDF will include ${medications.length} medicine${
                  medications.length !== 1 ? "s" : ""
                } - use browser print to save as PDF`
              : `PDF will include a complete plan for ${
                  medications.length
                } medicine${medications.length !== 1 ? "s" : ""}`}
          </ThemedText>
        </ThemedView>
      )}
    </ThemedView>
